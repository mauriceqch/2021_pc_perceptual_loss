...
# Perform the grouping of points by block with numba for a significant speedup (> x100)
@njit
...
def partition_octree(points, bbox_min, bbox_max, level):
    ...
    for x, y, z in block_ids_unique:
        ...
        assert len(set([len(s) for s in zip_params])) == 1, f'zip_params: {zip_params}, zyx: {z}, {y}, {x}, level: {level}, geo_level: {geo_level}, block_level: {block_level}'
